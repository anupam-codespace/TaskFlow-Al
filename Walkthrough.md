...
- **Relational Database**: A lightweight SQLite relational database is utilized via Flask-SQLAlchemy to store transactions. 

## 2. File Structure Boundaries
Maintaining clear separation of concerns:
- **`/backend/`**: Contains only the server-side environment (`app.py`, `requirements.txt`).
...
- **Simplicity Over Cleverness**: Backend responses are mapped directly to models via the `to_dict()` helper. The application does not pre-optimize with Redis caches or Docker networks since local scale does not necessitate it.
- **Interface Safety**: Inputs on the frontend `type="number"` with `min="0.01"` combined with backend validation (`if not data.get('title')...`) prevent invalid states implicitly (empty expenses or malformed queries). Float parsing and strict dictionary access guarantee type-safety on critical payloads.
